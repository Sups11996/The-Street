...
<%
    ...
    request.setAttribute("pageSubtitle", "View and manage all receiver requests.");
%>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Manage Requests – Sahayog Sathi</title>
    ...
    <style>
        ...
        .receiver-cell { display: flex; align-items: center; gap: 10px; }
        .receiver-avatar { width: 34px; height: 34px; border-radius: 50%; background: #D97706; color: #fff; display: flex; align-items: center; justify-content: center; font-weight: 700; font-size: 13px; flex-shrink: 0; }
        ...
        .urgency-badge { display: inline-block; padding: 3px 10px; border-radius: 20px; font-size: 11px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.4px; }
        ...
        .status-fulfilled { background: #D1FAE5; color: #065F46; } .status-fulfilled::before { background: #059669; }
    ...
    <main class="main">
        ...
        <div class="toolbar">
            <div class="search-box">
                ...
                <input type="search" id="searchInput" placeholder="Search by title, receiver or location..." oninput="filterTable()" autocomplete="off">
            </div>
            ...
            <span class="toolbar-count" id="rowCount">0 requests</span>
        </div>
            ...
            <div class="table-scroll">
                <table class="data-table" id="requestsTable">
                    ...
                    <tbody id="requestsBody">
                        <tr>
                            <td>1</td>
                            <td><div class="receiver-cell"><div class="receiver-avatar">R</div><div><div style="font-weight:600;">Raju Karki</div><div style="font-size:12px;color:#9CA3AF;">raju@receiver.com</div></div></div></td>
                            <td>Winter Clothes for Children</td>
                            <td><span class="category-badge cat-clothes">Clothes</span></td>
                            <td><span class="urgency-badge urgency-high">High</span></td>
                            <td style="color:#6B7280;">Pokhara</td>
                            <td style="color:#6B7280;">20 pieces</td>
                            <td><span class="status-badge status-pending">Pending</span></td>
                            <td>
                                <button class="btn-action btn-view" onclick="viewRequest(1)">
                                    <svg viewBox="0 0 24 24"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>View
                                </button>
                                <button class="btn-action btn-danger" onclick="confirmDelete(1, 'Winter Clothes for Children')" style="margin-left:4px;">
                        ...
                        <tr>
                            <td>2</td>
                            <td><div class="receiver-cell"><div class="receiver-avatar">K</div><div><div style="font-weight:600;">Kiran Shrestha</div><div style="font-size:12px;color:#9CA3AF;">kiran@receiver.com</div></div></div></td>
                            <td>Rice and Lentils for Shelter</td>
                            <td><span class="category-badge cat-food">Food</span></td>
                            <td><span class="urgency-badge urgency-medium">Medium</span></td>
                            <td style="color:#6B7280;">Kathmandu</td>
                            <td style="color:#6B7280;">50 kg</td>
                            <td><span class="status-badge status-accepted">Accepted</span></td>
                            <td>
                                <button class="btn-action btn-view" onclick="viewRequest(2)">
                                    <svg viewBox="0 0 24 24"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>View
                                </button>
                                <button class="btn-action btn-danger" onclick="confirmDelete(2, 'Rice and Lentils for Shelter')" style="margin-left:4px;">
                                    <svg viewBox="0 0 24 24"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/></svg>Delete
                                </button>
                            </td>
                        </tr>
                        <tr>
                            <td>3</td>
                            <td><div class="receiver-cell"><div class="receiver-avatar">G</div><div><div style="font-weight:600;">Gita Gurung</div><div style="font-size:12px;color:#9CA3AF;">gita@receiver.com</div></div></div></td>
                            <td>Blankets for Old Age Home</td>
                            <td><span class="category-badge cat-essentials">Essentials</span></td>
                            <td><span class="urgency-badge urgency-high">High</span></td>
                            <td style="color:#6B7280;">Lalitpur</td>
                            <td style="color:#6B7280;">15 pieces</td>
                            <td><span class="status-badge status-fulfilled">Fulfilled</span></td>
                            <td>
                                <button class="btn-action btn-view" onclick="viewRequest(3)">
                                    <svg viewBox="0 0 24 24"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>View
                                </button>
                                <button class="btn-action btn-danger" onclick="confirmDelete(3, 'Blankets for Old Age Home')" style="margin-left:4px;">
                                    <svg viewBox="0 0 24 24"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/></svg>Delete
                                </button>
                            </td>
                        </tr>
...
<div class="modal-overlay" id="deleteModal">
    <div class="modal">
        ...
        <div class="modal-body">
            ...
            <div class="modal-confirm-title">Delete this request?</div>
...
<script>
    ...
    function filterTable() {
        ...
        const urgency  = document.getElementById('urgencyFilter').value.toUpperCase();
        const status   = document.getElementById('statusFilter').value.toUpperCase();
        const allRows  = Array.from(document.querySelectorAll('#requestsTable tbody tr:not(.filler-row)'));

        filteredRows = allRows.filter(row => {
            const text  = row.textContent.toLowerCase();
            const cCell = row.querySelector('.category-badge') ? row.querySelector('.category-badge').textContent.trim().toUpperCase() : '';
            const uCell = row.querySelector('.urgency-badge')  ? row.querySelector('.urgency-badge').textContent.trim().toUpperCase()  : '';
            const sCell = row.querySelector('.status-badge')   ? row.querySelector('.status-badge').textContent.trim().toUpperCase()   : '';
            return (!search || text.includes(search)) && (!category || cCell.includes(category))
                && (!urgency || uCell.includes(urgency)) && (!status || sCell.includes(status));
        });
        currentPage = 1;
        renderPage();
    }
    ...
    function renderPage() {
        ...
        document.getElementById('rowCount').textContent = total + ' request' + (total !== 1 ? 's' : '');
    ...
    function viewRequest(id) { showToast('View details — backend coming soon.'); }
    ...
    function confirmDelete(id, title) {
        ...
        document.getElementById('confirmDeleteBtn').onclick = () => { closeModal(); showToast('Delete — backend coming soon.'); };
    ...
    document.addEventListener('DOMContentLoaded', () => {
        filteredRows = Array.from(document.querySelectorAll('#requestsTable tbody tr:not(.filler-row)'));
        renderPage();
    });
</script>
</body>
